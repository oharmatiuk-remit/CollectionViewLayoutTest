...
public func print(items: Any..., separator: String = " ", terminator: String = "\n") {
    ...
    let output = items.map { "\(dateString)\($0)"}.joined(separator: separator)
    Swift.print(output, terminator: terminator)
}

public func measure(label:String, callbackToExecute:()->()) {
    print(items:"[START] \(label)")
    
    let startDate = Date()
    callbackToExecute()
    let finishDate = Date()
    
    let diff = (finishDate.timeIntervalSince1970 - startDate.timeIntervalSince1970) * 1000.0
...
class LoremIpsumGenerator: NSObject {
    ...
    func createPhrase(_ wordsNumber:Int)->String {
        let offset = Int(arc4random_uniform(UInt32(loremIpsumText.characters.count / 2)))
        let initialIndex = loremIpsumText.index(loremIpsumText.startIndex, offsetBy: offset)
        var workText = String(loremIpsumText.split(separator: "\n").joined(separator: "").suffix(from: initialIndex))
        
        let startIndex = workText.index(workText.index(of: ".")!, offsetBy: 2) // index of first letter of the sentence
        workText = String(workText[startIndex...])
        
        let words = workText.split(separator: " ")[..<wordsNumber]
        var result = words.joined(separator: " ")
        result.removeLast()
        return result
    }
    
    private var loremIpsumText = """
